...
export default function FacturePrintPage() {
  ...
  // ✅ Ref pour éviter double impression
  ...
  if (error || !facture) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center bg-white rounded-xl p-8 max-w-md shadow-lg border-2 border-red-200">
  ...
  const totalTranchesPayees =
    facture.tranches?.filter((t: any) => t.statut === 'paye').length || 0;
            ...
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-2xl font-bold text-gray-800">
                  {facture.type_document === 'proformat'
                    ? 'PROFORMAT'
                    : 'FACTURE'}
                </h1>
              ...
              <div className="text-right">
                ...
                <span
                  className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${getStatutBadge(
                    facture.statut
                  )}`}
                >
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            ...
            <div className="text-right">
              ...
              <p className="font-bold">
                {facture.commercial_prenom} {facture.commercial_nom}
              </p>
              <p className="text-sm text-gray-600">
                {facture.commercial_email || ''}
              </p>
            </div>
          </div>
                  ...
                  <tr key={index} className="border-t">
                    <td className="p-3">
                      <p className="font-medium">
                        {ligne.panneau_nom || 'N/A'}
                      </p>
                      <p className="text-sm text-gray-500">
                        Face {ligne.face_orientation || 'N/A'}
                      </p>
                      {ligne.reservation_numero && (
                        <p className="text-xs text-gray-400">
                          Réservation: {ligne.reservation_numero}
                        </p>
                      )}
                    </td>
              ...
              <div className="space-y-2">
                ...
                {facture.remise > 0 && (
                  <div className="flex justify-between text-green-600">
                    ...
                    <span>
                      - {facture.montant_remise?.toLocaleString() || '0'} FCFA
                    </span>
                  </div>
                )}
                <div className="flex justify-between text-lg font-bold border-t pt-2">
                  ...
                  <span className="text-blue-600">
                    {facture.total_ttc?.toLocaleString() || '0'} FCFA
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="mt-6 border-t pt-6">
            <div className="bg-gray-50 rounded-lg p-4">
              <h4 className="font-bold text-gray-700 mb-3 flex items-center gap-2">
                <CreditCard size={18} />
                {facture.mode_paiement === 'tranche'
                  ? 'Paiement en tranches'
                  : 'Paiement comptant'}
              </h4>

              {facture.mode_paiement === 'tranche' &&
                facture.nombre_tranches > 1 && (
                  <div>
                    <div className="flex justify-between items-center mb-3">
                      <span className="text-sm text-gray-600">
                        {facture.nombre_tranches} tranche(s) de{' '}
                        {facture.montant_par_tranche?.toLocaleString() || '0'}{' '}
                        FCFA
                      </span>
                      <span className="text-sm font-medium">
                        Payé: {facture.montant_paye?.toLocaleString() || '0'}{' '}
                        FCFA
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      {facture.tranches?.map((tranche: any) => (
                        <div
                          key={tranche.id_tranche}
                          className="flex justify-between items-center p-2 bg-white rounded border"
                        >
                          <div className="flex items-center gap-3">
                            <span className="font-medium">
                              Tranche {tranche.numero_tranche}
                            </span>
                            <span className="text-sm text-gray-600">
                              Échéance:{' '}
                              {new Date(
                          ...
                          <div className="flex items-center gap-4">
                            <span className="font-medium">
                              {tranche.montant?.toLocaleString() || '0'} FCFA
                            </span>
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-bold ${getTrancheStatutBadge(
                                tranche.statut
                              )}`}
                            >
                              {tranche.statut === 'paye'
                                ? '✅ Payé'
                                : tranche.statut === 'retard'
                                ? '⏰ Retard'
                                : '⏳ En attente'}
                            </span>
